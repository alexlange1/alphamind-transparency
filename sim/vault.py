...
# Ring buffer per uid of (timestamp, price)
_price_obs: Dict[int, deque] = defaultdict(lambda: deque(maxlen=2048))

def record_price(uid: int, price: float, ts: Optional[float] = None) -> None:
    ts = ts or time.time()
    _price_obs[int(uid)].append((float(ts), float(price)))

def get_twap(uid: int, window_sec: Optional[int] = None, now_ts: Optional[float] = None) -> float:
    from ..common.settings import get_settings
    settings = get_settings()
    window = window_sec or int(settings.twap_window_min * 60)
    now = now_ts or time.time()
    start = now - window
    obs = _price_obs.get(int(uid))
    if not obs:
    ...
    for ts, p in obs:
        ...
        if dt > 0:
            ...
            den += dt
        last_ts, last_p = ts, p
    if last_ts is not None and last_p is not None and now > last_ts:
        num += last_p * (now - last_ts)
        den += (now - last_ts)
    if den == 0:
        # Fallback to last observed price if window too sparse
        return obs[-1][1]
    return num / den
